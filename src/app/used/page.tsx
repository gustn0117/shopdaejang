  ...
  return (
    <div className="container-custom py-4 lg:py-6">
      <div className="mb-4 flex items-end justify-between">
        <div>
          <h1 className="text-xl lg:text-2xl font-black">업주 직거래 중고장터</h1>
          <p className="text-xs lg:text-sm text-muted mt-1">
            마사지샵 용품과 집기를 업주들끼리 직거래하세요
          </p>
        </div>
        <Link href="/used/write" className="px-4 py-2 bg-primary text-white text-xs lg:text-sm font-bold rounded-lg">
          + 글쓰기
        </Link>
      </div>
      ...
      <div className="bg-white rounded-xl border border-border overflow-hidden">
              ...
              return (
                <Link
                  ...
                  className={`px-3 py-1.5 text-xs font-bold rounded ${
                    active ? "bg-primary text-white" : "bg-zinc-100 text-muted hover:bg-zinc-200"
                  }`}
                >
                  {t.l}
                </Link>
        ...
        <ul className="divide-y divide-border">
          {items.map((item) => (
            <li key={item.id}>
              <Link href={`/used/${item.id}`} className="flex gap-3 p-3 hover:bg-primary-light">
                <div className="relative w-20 h-20 sm:w-24 sm:h-24 shrink-0 rounded-lg bg-zinc-100 overflow-hidden">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1 mb-1">
                    <span className={`px-1.5 py-0.5 text-[10px] font-bold rounded ${
                      item.category === "팝니다" ? "bg-primary text-white" : "bg-blue-500 text-white"
                    }`}>
                      {item.category}
                    </span>
      ...
      <div className="mt-4 text-[11px] text-muted bg-amber-50 border border-amber-200 rounded-lg p-3">
        <p className="font-bold mb-1">💡 안전 거래 팁</p>
