...
export default async function UsedDetailPage({
  ...
  return (
    <div className="container-custom py-4 lg:py-6 max-w-3xl">
      <Link href="/used" className="text-xs text-muted hover:text-primary">← 중고장터 목록</Link>
      ...
      <div className="bg-white rounded-xl border border-border overflow-hidden mt-3">
        ...
        <div className="p-4 lg:p-6">
          ...
          <h1 className="text-xl lg:text-2xl font-black mb-2">{item.title}</h1>
          <p className="text-2xl lg:text-3xl font-black text-primary mb-4">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <button className="py-3 bg-foreground text-white font-bold rounded-lg">
          📞 전화걸기
        </button>
        <button className="py-3 bg-[#FEE500] text-black font-bold rounded-lg">
          💬 카톡 채팅
        </button>
      </div>

      <div className="mt-3 text-center">
        <button className="text-xs text-muted hover:text-red-600">🚨 신고하기</button>
      </div>
    </div>
  );
}
