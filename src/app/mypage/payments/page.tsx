...
export default function PaymentsPage() {
  ...
  return (
    <div className="space-y-3">
      <h1 className="text-lg lg:text-xl font-black">결제내역</h1>
      ...
      <div className="grid grid-cols-3 gap-2 lg:gap-3">
        ...
        <div className="bg-white rounded-xl border border-border p-3 lg:p-4">
          ...
          <p className="text-lg lg:text-2xl font-black mt-1 text-primary">{total.toLocaleString()}원</p>
        </div>
        <div className="bg-white rounded-xl border border-border p-3 lg:p-4">
            ...
            <li key={p.id} className="grid grid-cols-2 md:grid-cols-[160px_1fr_120px_100px_100px_80px] gap-2 px-3 py-3 text-sm items-center">
              ...
              <div className="text-center">
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                  p.status === "결제완료" ? "bg-emerald-100 text-emerald-700" : "bg-zinc-100 text-zinc-700"
                }`}>
                  {p.status}
                </span>
              </div>
              <div className="text-center">
                <button className="text-[11px] text-primary hover:underline">영수증</button>
              </div>
            </li>
          ))}
        </ul>
