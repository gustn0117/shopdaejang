...
export default function AdConfigPage() {
  return (
    <div className="space-y-3">
      <h1 className="text-lg lg:text-xl font-black tracking-tight">광고 상품별 노출순서 관리</h1>
      ...
      <div className="grid gap-3">
        {AD_PRICING.map((p, idx) => (
          <div key={p.tier} className="bg-white rounded-md border border-border p-4 lg:p-5">
      ...
      <div className="flex justify-end gap-2">
        <button type="button" className="px-4 py-2 border border-border text-sm font-bold rounded">취소</button>
        <button type="button" className="px-5 py-2 bg-foreground text-white text-sm font-bold rounded">설정 저장</button>
      </div>
    </div>
  );
}
