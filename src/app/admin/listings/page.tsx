  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h1 className="text-lg lg:text-xl font-black">매물 승인 관리</h1>
        ...
        {[
          ...
          <button
            ...
            className={`shrink-0 px-3 py-1.5 text-xs font-bold rounded ${
              t.active ? "bg-primary text-white" : "text-muted hover:bg-zinc-50"
            }`}
          >
            {t.l}
          </button>
        ...
        <ul className="divide-y divide-border">
          {pending.map((l) => (
            <li key={l.id} className="px-3 py-3 grid grid-cols-1 lg:grid-cols-[40px_80px_1fr_120px_100px_100px_180px] gap-2 items-center text-sm">
              <div className="hidden lg:block"><input type="checkbox" /></div>
              ...
              <div className="text-xs hidden lg:block">
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                  l.tier === "free" ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"
                }`}>
                  {l.tier === "free" ? "무료 10일" : "결제완료"}
                </span>
              </div>
              <div className="grid grid-cols-3 gap-1">
                <button className="py-1.5 text-xs font-bold bg-emerald-500 text-white rounded">승인</button>
                <button className="py-1.5 text-xs font-bold bg-red-500 text-white rounded">반려</button>
                <button className="py-1.5 text-xs font-bold border border-border rounded">상세</button>
              </div>
            </li>
          ))}
        </ul>
      ...
      <div className="bg-white border border-border rounded-xl p-3 flex items-center justify-between">
        ...
        <div className="flex gap-1">
          <button className="px-3 py-1.5 text-xs font-bold bg-emerald-500 text-white rounded">선택 승인</button>
          <button className="px-3 py-1.5 text-xs font-bold bg-red-500 text-white rounded">선택 반려</button>
        </div>
      </div>
    </div>
  );
